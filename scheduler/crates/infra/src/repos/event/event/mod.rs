...
mod tests {
    ...
    async fn create_and_delete() {
        for ctx in create_contexts().await {
            ...
            // Insert
            assert!(ctx.repos.event_repo.insert(&event).await.is_ok());

            // Different find methods
            let get_event_res = ctx.repos.event_repo.find(&event.id).await.unwrap();
            ...
            // Find
            assert!(ctx.repos.event_repo.find(&event.id).await.is_none());
        }
    }
    ...
    async fn update() {
        for ctx in create_contexts().await {
            ...
            // Save
            assert!(ctx.repos.event_repo.save(&event).await.is_ok());
    ...
    async fn delete_by_user() {
        for ctx in create_contexts().await {
            ...
            // Delete
            let res = ctx.repos.event_repo.delete_by_user(&event.user_id).await;
            assert!(res.is_ok());
